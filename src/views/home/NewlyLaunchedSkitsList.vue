...
import { Putnewlist, Delnewlist, Getnewlist } from '@/api/home/home'
export default {
  ...
  computed: {
    ...
    paginatedTVShows () {
      ...
      const end = start + this.numpageSize
      return this.tvShows.slice(start, end)
    }
  },
  mounted () {
    this.GetVedioList()
  ...
  methods: {
    ...
    async addSelectedSkits () {
      ...
      const res = await Putnewlist(this.selectedTVShows)
      console.log(res)
      this.Getreclist()
    ...
    async deleteSkit (skit) {
      ...
      const res = await Delnewlist(this.selectedIds)
      if (res.code === 200) {
        this.Getreclist()
